...
# --------------------------------------------------------
from src.utils import load_json, OrderedDict, critical, join, ufloat, choose, array, BaseDir, load, loads
...
class DUT:
    ...
    def load_irradiation(self):
        with open(join(BaseDir, self.Config.get('MISC', 'irradiation file'))) as f:
            data = load(f)
